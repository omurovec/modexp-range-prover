...
mod tests {
    ...
    fn test_mod_exp() {
        let base: BigUint = BigUint::parse_bytes(b"4", 10).unwrap();
        let modulus: BigUint = BigUint::parse_bytes(b"497", 10).unwrap();
        let range: BigUint = modulus.clone() / BigUint::parse_bytes(b"14", 10).unwrap();
        let exp: BigUint = BigUint::parse_bytes(b"13", 10).unwrap();
        ...
        assert_eq!(result, BigUint::parse_bytes(b"445", 10).unwrap());
    }
    ...
    fn test_invalid_exp() {
        let base: BigUint = BigUint::parse_bytes(b"4", 10).unwrap();
        let modulus: BigUint = BigUint::parse_bytes(b"497", 10).unwrap();
        let range: BigUint = BigUint::parse_bytes(b"12", 10).unwrap();
        let exp: BigUint = BigUint::parse_bytes(b"13", 10).unwrap();
